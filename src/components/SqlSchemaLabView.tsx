  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-[#16191F] border border-[#2D3139] rounded-2xl p-5 space-y-3 shadow-xl">
          ...
          <div className="space-y-2 text-xs font-mono text-[#E0E6ED]">
            <div className="p-2.5 bg-[#0F1115] rounded-xl border border-[#2D3139] flex items-center justify-between">
              <span>Support a Chaves Estrangeiras (`PRAGMA foreign_keys = ON`)</span>
              <span className="text-[#00E676] font-bold">✓ Ativo</span>
            </div>
            <div className="p-2.5 bg-[#0F1115] rounded-xl border border-[#2D3139] flex items-center justify-between">
              <span>8 Tabelas do Modelo Relacional</span>
              <span className="text-[#00E676] font-bold">✓ Válido</span>
            </div>
            <div className="p-2.5 bg-[#0F1115] rounded-xl border border-[#2D3139] flex items-center justify-between">
              <span>Índices de Performance por Categoria e Pote</span>
              <span className="text-[#00E676] font-bold">✓ Otimizado</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#16191F] border border-[#2D3139] rounded-2xl p-5 space-y-3 shadow-xl">
          ...
          <div className="space-y-2 text-xs font-mono text-[#E0E6ED]">
            <div className="p-2.5 bg-[#0F1115] rounded-xl border border-[#2D3139]">
            ...
            <div className="p-2.5 bg-[#0F1115] rounded-xl border border-[#2D3139]">
              <p className="font-bold text-blue-400">Processador de Suspensões por Cartão</p>
              <p className="text-[11px] text-[#8E9299] font-sans mt-0.5">
      ...
      <div className="bg-[#16191F] border border-[#2D3139] rounded-2xl p-5 space-y-4 shadow-xl">
        ...
        <div className="flex flex-wrap gap-2">
          {predefinedQueries.map((q, idx) => (
            <button
              ...
              className="px-2.5 py-1 bg-[#0F1115] hover:bg-[#2D3139] text-[#E0E6ED] hover:text-white rounded-lg text-[11px] font-mono font-semibold border border-[#2D3139] transition-colors"
            >
              {q.label}
            </button>
          ))}
        ...
        <div className="space-y-2">
          <textarea
            ...
            className="w-full bg-[#0F1115] text-[#00E676] font-mono text-xs rounded-xl p-3 border border-[#2D3139] focus:outline-none focus:ring-1 focus:ring-[#00E676]"
          />

          <div className="flex justify-end">
            <button
              onClick={handleExecuteSql}
              className="px-4 py-2 bg-[#00E676] hover:bg-[#00c853] text-black font-extrabold text-xs uppercase tracking-wider rounded-xl flex items-center space-x-1.5 shadow-[0_0_15px_rgba(0,230,118,0.3)] transition-all"
            >
        ...
        {queryResult && (
          <div className="space-y-2">
            ...
            <div className="bg-[#0F1115] rounded-xl border border-[#2D3139] overflow-x-auto max-h-[300px]">
              {queryResult.length === 0 ? (
                <p className="p-4 text-xs text-[#8E9299] font-mono text-center">Nenhum registro retornado.</p>
              ) : (
                <table className="w-full text-left text-xs font-mono">
                  <thead className="bg-[#16191F] text-[#8E9299] border-b border-[#2D3139] text-[10px] tracking-wider uppercase font-bold">
                    <tr>
                      {Object.keys(queryResult[0]).map((key) => (
                        <th key={key} className="py-2 px-3 border-r border-[#2D3139]">
                          {key}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody className="divide-y divide-[#2D3139]">
                    {queryResult.map((row, rIdx) => (
                      <tr key={rIdx} className="hover:bg-[#16191F]/50 transition-colors">
                        {Object.values(row).map((val: any, cIdx) => (
                          <td key={cIdx} className="py-2 px-3 text-[#E0E6ED] border-r border-[#2D3139]/50 truncate max-w-[200px]">
